...
{
    public CallExpression(FunctionDeclarationExpression functionDeclarationExpression, Location location) : base(location)
    {
        FunctionDeclarationExpression = functionDeclarationExpression;
        _functionCall = new FunctionCallExpression(string.Empty, Array.Empty<AbstractEvaluableExpression>(), null, Location);
    }
